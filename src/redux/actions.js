...
const URL = "https://dogs-api-jk7r.onrender.com";
...
export const getTempDB = () => {
	return async function (dispatch) {
		try {
			let res = await axios.get(`${URL}/temperaments`),
				json = await res.data;
			return dispatch({
				type: GET_TEMP_DB,
				payload: json,
...
export const createNewRace = (data) => {
	return async function () {
		try {
			const res = await axios.post(`${URL}/dogs`, data);
...
export const deleteDog = (name) => {
	return async function () {
		try {
			const res = await axios.delete(`${URL}/dogs?name=${name}`);
